...
/**
 * MATLAB interface: s2let_axisym_mw_analysis_mex.
 ...
 * Usage: 
 *   [f_wav, f_scal] = ...
 *        s2let_axisym_mw_analysis_mex(f, B, L, J_min, reality);
 *
 */
                  ...
                  int nrhs, const mxArray *prhs[])
{
  ...
  if(nrhs!=6) {
    mexErrMsgIdAndTxt("s2let_axisym_mw_analysis_mex:InvalidInput:nrhs",
          "Require six inputs.");
  }
  if(nlhs!=2) {
    mexErrMsgIdAndTxt("s2let_axisym_mw_analysis_mex:InvalidOutput:nlhs",
          "Require two outputs.");
  }
  ...
  if( !mxIsLogicalScalar(prhs[iin]) )
    mexErrMsgIdAndTxt("s2let_axisym_mw_analysis_mex:InvalidInput:reality",
  ...
  iin = 5;
  if( !mxIsLogicalScalar(prhs[iin]) )
    mexErrMsgIdAndTxt("s2let_axisym_mw_analysis_mex:InvalidInput:downsample",
  ...
  if( !mxIsDouble(prhs[iin]) || 
    ...
    mexErrMsgIdAndTxt("s2let_axisym_mw_analysis_mex:InvalidInput:waveletParameter",
  ...
  if( !mxIsDouble(prhs[iin]) || 
      mxIsComplex(prhs[iin]) || 
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("s2let_axisym_mw_analysis_mex:InvalidInput:LbandLimit",
          "Harmonic band-limit L must be integer.");
  }
  L = (int)mxGetScalar(prhs[iin]);

  if (mxGetScalar(prhs[iin]) > (double)L || L <= 0)
    mexErrMsgIdAndTxt("s2let_axisym_mw_analysis_mex:InvalidInput:bandLimitNonInt",
  ...
  if( !mxIsDouble(prhs[iin]) || 
      mxIsComplex(prhs[iin]) || 
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("s2let_axisym_mw_analysis_mex:InvalidInput:Jmin",
  ...
  if(downsample){
    ...
    if(reality){
      s2let_axisym_mw_allocate_f_wav_multires_real(&f_wav_r, &f_scal_r, B, L, J_min);
      s2let_axisym_mw_wav_analysis_multires_real(f_wav_r, f_scal_r, f_r, B, L, J_min);
    }else{
    ...
    if(reality){
      s2let_axisym_mw_allocate_f_wav_real(&f_wav_r, &f_scal_r, B, L, J_min);
      s2let_axisym_mw_wav_analysis_real(f_wav_r, f_scal_r, f_r, B, L, J_min);
    }else{
