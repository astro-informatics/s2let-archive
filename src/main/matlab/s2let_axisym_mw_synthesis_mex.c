 ...
 * Usage: 
 *   f = ...
 *        s2let_axisym_mw_synthesis_mex(f_wav, f_scal, B, L, J_min, reality);
 *
 */
                  ...
                  int nrhs, const mxArray *prhs[])
{
  ...
  if(nrhs!=7) {
    mexErrMsgIdAndTxt("s2let_axisym_mw_synthesis_mex:InvalidInput:nrhs",
          "Require seven inputs.");
  }
  if(nlhs!=1) {
    mexErrMsgIdAndTxt("s2let_axisym_mw_synthesis_mex:InvalidOutput:nlhs",
          "Require two outputs.");
  }
  ...
  if( !mxIsLogicalScalar(prhs[iin]) )
    mexErrMsgIdAndTxt("s2let_axisym_mw_synthesis_mex:InvalidInput:reality",
  ...
  iin = 6;
  if( !mxIsLogicalScalar(prhs[iin]) )
    mexErrMsgIdAndTxt("s2let_axisym_mw_synthesis_mex:InvalidInput:downsample",
  ...
  if( !mxIsDouble(prhs[iin]) || 
    ...
    mexErrMsgIdAndTxt("s2let_axisym_mw_synthesis_mex:InvalidInput:waveletParameter",
  ...
  if( !mxIsDouble(prhs[iin]) || 
      mxIsComplex(prhs[iin]) || 
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("s2let_axisym_mw_synthesis_mex:InvalidInput:LbandLimit",
          "Harmonic band-limit L must be integer.");
  }
  L = (int)mxGetScalar(prhs[iin]);

  if (mxGetScalar(prhs[iin]) > (double)L || L <= 0)
    mexErrMsgIdAndTxt("s2let_axisym_mw_synthesis_mex:InvalidInput:bandLimitNonInt",
  ...
  if( !mxIsDouble(prhs[iin]) || 
      mxIsComplex(prhs[iin]) || 
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("s2let_axisym_mw_synthesis_mex:InvalidInput:Jmin",
  ...
  if(downsample){
    ...
    if(reality){
      ...
      s2let_axisym_mw_wav_synthesis_multires_real(f_r, f_wav_r, f_scal_r, B, L, J_min);
    }else{
    ...
    if(reality){
      s2let_mw_allocate_real(&f_r, L);
      s2let_axisym_mw_wav_synthesis_real(f_r, f_wav_r, f_scal_r, B, L, J_min);
    }else{
      s2let_mw_allocate(&f, L);
