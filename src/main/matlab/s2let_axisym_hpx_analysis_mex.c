 ...
 * Usage: 
 *   [f_wav, f_scal] = ...
 *        s2let_axisym_hpx_analysis_mex(f, nside, B, L, J_min);
 *
 */
                  ...
                  int nrhs, const mxArray *prhs[])
{
  ...
  if(nrhs!=5) {
    mexErrMsgIdAndTxt("s2let_axisym_hpx_analysis_mex:InvalidInput:nrhs",
          "Require six inputs.");
  }
  if(nlhs!=2) {
    mexErrMsgIdAndTxt("s2let_axisym_hpx_analysis_mex:InvalidOutput:nlhs",
          "Require two outputs.");
  }
  ...
  if( !mxIsDouble(prhs[iin]) || 
    ...
    mexErrMsgIdAndTxt("s2let_axisym_hpx_analysis_mex:InvalidInput:healpixParameter",
  ...
  if( f_m*f_n != 12*nside*nside ) 
    mexErrMsgIdAndTxt("s2let_axisym_hpx_analysis_mex:InvalidInput:LbandLimit",
  ...
  if( !mxIsDouble(prhs[iin]) || 
      mxIsComplex(prhs[iin]) || 
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("s2let_axisym_hpx_analysis_mex:InvalidInput:waveletParameter",
  ...
  if (mxGetScalar(prhs[iin]) > (double)L || L <= 0)
    mexErrMsgIdAndTxt("s2let_axisym_hpx_analysis_mex:InvalidInput:bandLimitNonInt",
  ...
  if( !mxIsDouble(prhs[iin]) || 
      mxIsComplex(prhs[iin]) || 
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("s2let_axisym_hpx_analysis_mex:InvalidInput:Jmin",
