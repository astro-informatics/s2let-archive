 ...
 * Usage: 
 *   f = ...
 *        s2let_hpx_axisym_synthesis_mex(f_wav, f_scal, nside, B, L, J_min);
 *
 */
                  ...
                  int nrhs, const mxArray *prhs[])
{
  ...
  if( !mxIsDouble(prhs[iin]) || 
      mxIsComplex(prhs[iin]) || 
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("s2let_hpx_axisym_synthesis_mex:InvalidInput:waveletParameter",
  ...
  if( !mxIsDouble(prhs[iin]) || 
      mxIsComplex(prhs[iin]) || 
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("s2let_hpx_axisym_synthesis_mex:InvalidInput:Jmin",
