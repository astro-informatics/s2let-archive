...
/**
 * MATLAB interface: s2let_axisym_tilling_mex.
 * This function for internal use only.
 * Compute tilling in harmonic space for axisymmetric wavelets.
 *
 * Usage: 
                  ...
                  int nrhs, const mxArray *prhs[])
{
  ...
  if(nrhs!=3) {
    mexErrMsgIdAndTxt("s2let_axisym_tilling_mex:InvalidInput:nrhs",
		      "Require three inputs.");
  }
  if(nlhs!=2) {
    mexErrMsgIdAndTxt("s2let_axisym_tilling_mex:InvalidOutput:nlhs",
		      "Require two outputs.");
  }
  ...
  if( !mxIsDouble(prhs[iin]) || 
    ...
    mexErrMsgIdAndTxt("s2let_axisym_tilling_mex:InvalidInput:waveletParameter",
          "Wavelet parameter B must be integer.");
  }
  B = (int)mxGetScalar(prhs[iin]);
  if (mxGetScalar(prhs[iin]) > (double)B || B <= 1)
    mexErrMsgIdAndTxt("s2let_axisym_tilling_mex:InvalidInput:bandLimitNonInt",
  ...
  if( !mxIsDouble(prhs[iin]) || 
      mxIsComplex(prhs[iin]) || 
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("s2let_axisym_tilling_mex:InvalidInput:LbandLimit",
