function s2let_axisym_mw_wav_synthesis, f_wav, verbose=verbose
;+
...
if s2let_dylib_exists() eq 1 then begin
   ...
   print, '=========================================='
   print, 's2let_axisym_mw_wav_synthesis'
   print, '------------------------------------------'
   help, L, B, J_min, J_max
   endif

   f_scal = dcomplex(dblarr(npix))
   f_scal(0:npix-1) = dcomplex(f_wav.scal) ;f_wav(0:npix-1, 0)
   f_wav_vec = dcomplex(dblarr((J_max+1-J_min)*npix))
   for j=0, J_max-J_min do begin
       ;jnm = 'j'+strtrim(j,2)
       f_wav_vec( j*npix : (j+1)*npix-1 ) = f_wav.(j);jnm ;f_wav(0:npix-1, j+1)
   endfor

   f = dcomplex(dblarr(npix))
   r = call_external(soname, 's2let_idl_transform_axisym_wav_synthesis_mw', f, dcomplex(f_wav_vec), dcomplex(f_scal), B, L, J_min, wavtype, /CDECL)
