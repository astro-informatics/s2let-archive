...
def pys2let_transform_axisym_lm_wav_analysis(
	np.ndarray[double complex, ndim=1, mode="c"] flm_real not None, B, L, J_min):
	...
	print flm_real.shape[0], f_lm.shape[0]

	for el from 0 <= el < L:
		for em from 0 <= em <= el:
			#print el, em, el * (el + 1)/2 + em, el * el + el + em, el * el + el - em
			f_lm[ el * el + el - em ] = pow(-1.0, -em) * ( flm_real[healpy_lm(el, em, L)] ).conjugate()
			f_lm[ el * el + el + em ] = flm_real[healpy_lm(el, em, L)]
	...
	f_scal_lm_real = np.empty([L*(L+1)/2,], dtype=complex)
	f_wav_lm_real = np.empty([L*(L+1)/2, J-J_min+1], dtype=complex)

	for el from 0 <= el < L:
		for em from 0 <= em <= el:
			#print el, em, el * (el + 1)/2 + em, el * el + el + em, el * el + el - em
			f_scal_lm_real[healpy_lm(el, em, L)] = f_scal_lm[ el * el + el + em ]
			for j from 0 <= j <= J-J_min:
				f_wav_lm_real[healpy_lm(el, em, L), j] = f_wav_lm[ L*L*j + el * el + el + em ]

	return f_wav_lm_real, f_scal_lm_real
